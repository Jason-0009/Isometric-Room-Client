 ...
 * @returns {Point3D} An object containing the converted isometric coordinates (x, y, z).
 */
export const cartesianToIsometric = (position: Point3D): Point3D => {
    const x: number = (position.x - position.y) * (TILE_DIMENSIONS.WIDTH / 2)
    const y: number = (position.x + position.y) * (TILE_DIMENSIONS.HEIGHT / 2)
